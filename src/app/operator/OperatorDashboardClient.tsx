...
export default function OperatorDashboardClient({ properties, allProperties, availability, pendingRequests: initialPending }: Props) {
    ...
    return (
        <div className="space-y-4 w-full min-w-0">
            {/* Tabs */}
            <div className="flex items-center gap-3 overflow-x-auto whitespace-nowrap border-b border-gray-200 no-scrollbar pb-1 -mx-3 px-3 sm:mx-0 sm:px-0">
                <button 
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-6">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-72 overflow-y-auto pr-1">
                                {allProperties.map(p => {
                                    const isSelected = availPropertyId === p.id;
                                    return (
                                        <button
                                            key={p.id}
                                            type="button"
                                            onClick={() => { setAvailPropertyId(p.id); setAvailSelection({ start: null, end: null }); setAvailSuccess(''); setAvailRoomId(''); }}
                                            className={`relative rounded-xl overflow-hidden border-2 transition-all text-left ${isSelected ? 'border-green-500 ring-2 ring-green-200' : 'border-gray-100 hover:border-gray-300'}`}
                                        >
                                            ...
                                            <div className="p-2">
                                                ...
                                                <p className="text-[10px] text-gray-500 truncate">{p.area || p.city}</p>
                                            </div>
                                        </button>
                                    );
                                })}
                                ...
                                {availSelection.start && (
                                    <div className="mt-4 p-3 bg-gray-50 rounded-xl animate-in slide-in-from-top-2 duration-200">
                                        <p className="text-sm font-medium text-gray-900 mb-1">
                                            <span className="font-bold">{availSelection.start}</span>
                                            {availSelection.end && <span className="text-gray-500"> → <span className="font-bold text-gray-900">{availSelection.end}</span></span>}
                                        </p>
                                        <p className="text-xs text-gray-500 mb-3">
                                            {!availSelection.end ? 'Tap another date to set end, or choose action below' : 'Range selected'}
                                        </p>
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                onClick={() => { setAvailBookingType('guest'); setAvailBlockForm({ guestName: '', notes: '' }); setShowAvailBlockModal(true); }}
                                                className="py-2.5 rounded-xl text-xs font-semibold bg-gray-900 text-white hover:bg-gray-800 transition-colors col-span-2"
                                            >
                                                Block Dates
                                            </button>
                                            <button
                                                ...
                                                className="py-2.5 rounded-xl text-xs font-medium bg-green-100 text-green-700 hover:bg-green-200 transition-colors disabled:opacity-50"
                                            >
                                                Mark Available
                                            </button>
                                            <button
                                                onClick={() => { setAvailSelection({ start: null, end: null }); }}
                                                className="py-2.5 rounded-xl text-xs font-medium bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors"
                                            >
                                                Clear
                                            </button>
                                        </div>
                                ...
                                <div key={p.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden flex">
                                    ...
                                    <div className="relative w-28 shrink-0 bg-gray-100">
                                        ...
                                        <div className="absolute top-2 left-2">
                                            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${p.is_active ? 'bg-green-500 text-white' : 'bg-gray-700 text-gray-200'}`}>
                                    ...
                                    <div className="flex-1 min-w-0 p-3 flex flex-col justify-between">
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-green-700 mt-1">
                                                ₦{p.price_per_night.toLocaleString()}<span className="text-xs text-gray-400 font-normal">/night</span>
                                            </p>
                                        </div>
                                        <button
                                            onClick={() => openPropEdit(p)}
                                            className="mt-2 flex items-center justify-center gap-1.5 py-1.5 text-xs font-medium text-gray-700 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors border border-gray-100 w-full"
                                        >
                                            <Pencil size={11} />
                                            Edit
                                        </button>
