...
const navItems = [
    { href: '/operator', label: 'Call Dashboard', icon: PhoneCall },
];

export default function OperatorShell({ children, userName }: { children: React.ReactNode; userName?: string }) {
            ...
            >
                <div className="flex flex-col h-full">
                    <div className="p-6 border-b border-gray-100 flex-1">
                        ...
                        <nav className="mt-6 flex-1 space-y-1 relative">
                            {navItems.map((item) => {
                    ...
                    <div className="p-4 border-t border-gray-100 bg-gray-50/50">
                        <div className="flex items-center gap-3 mb-3">
                            <div className="w-9 h-9 bg-green-100 rounded-full flex items-center justify-center text-green-600 text-sm font-bold border border-green-200">
                                {userName?.[0] || 'O'}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-gray-900 truncate">{userName || 'Operator'}</p>
                                <p className="text-xs text-green-500 font-medium">Call Operator</p>
                            </div>
                        </div>
                        <button
